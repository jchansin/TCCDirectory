...
export class DatabaseProvider {
    ...
    public insertFavorite(x, y): any {
        this.db.executeSql(
            "INSERT INTO `favorites` (business_id, name, favorite) VALUES (" + x + ", " + y + ", 1);", {}
        )
    ...
    public toggleFavorite(x, y, z): any {
        if (z == 1) {
            ...
            this.db.executeSql(
                "UPDATE `favorites` SET favorite=1 WHERE business_id=" + x + ";", {}
            )
    ...
    public getFavoritesList(): any {
        this.db.executeSql(
            "SELECT * FROM favorites WHERE favorite='1';", {}
        )
        .then((data) => {
            console.log(data);
            return data;
        })
        .catch(e => console.log('Erreur dans getFavoritesList', JSON.stringify(e)))
    }
